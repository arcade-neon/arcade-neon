  ...
  return (
    <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-4 relative overflow-hidden font-mono">
      <div className="absolute top-6 right-6 z-50">
        <button onClick={() => setShowLangMenu(!showLangMenu)} className="flex items-center gap-2 bg-slate-900/80 border border-slate-700 px-4 py-2 rounded-full text-white hover:border-emerald-500 transition-colors">
      ...
      <div className="z-10 bg-slate-900/80 border border-slate-700 p-8 rounded-3xl shadow-2xl backdrop-blur-md w-full max-w-md text-center animate-in zoom-in duration-500">
        ...
        <div className="space-y-4">
          <button onClick={handleGoogle} disabled={loading} className="w-full bg-white text-black font-bold py-4 rounded-xl flex items-center justify-center gap-3 hover:scale-105 transition-transform">{loading ? <Zap className="w-5 h-5 animate-spin" /> : t.googleBtn}</button>
          ...
          <button onClick={handleGuest} disabled={loading} className="w-full bg-slate-800 text-slate-300 font-bold py-4 rounded-xl flex items-center justify-center gap-3 hover:bg-slate-700 transition-colors border border-slate-700"><Ghost className="w-5 h-5" /> {t.guestBtn}</button>
        </div>
...
const Dashboard = ({ user }: { user: any }) => {
  ...
  useEffect(() => {
    const checkNickname = async () => {
        ...
        } catch (error) { console.error("Error apodo:", error); }
    };
    checkNickname();
  }, [user]);
  ...
  return (
    <div className="min-h-screen bg-[#050b14] flex flex-col items-center p-4 font-mono select-none overflow-x-hidden">
      ...
      {showNicknameModal && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in zoom-in-95">
            <div className="bg-[#0f172a] border border-emerald-500/50 p-8 rounded-3xl w-full max-w-sm text-center shadow-[0_0_60px_rgba(16,185,129,0.2)]">
                ...
                <input type="text" placeholder="TU APODO..." value={nickname} onChange={(e) => setNickname(e.target.value)} maxLength={12} className="w-full bg-slate-950 border-2 border-slate-700 text-white text-center font-black text-xl py-4 rounded-2xl focus:border-emerald-500 outline-none uppercase mb-4" autoFocus />
                <button onClick={handleSaveNickname} disabled={savingNick || nickname.length < 3} className="w-full bg-emerald-600 text-white font-black py-4 rounded-2xl shadow-lg flex items-center justify-center gap-2">{savingNick ? <Loader2 className="animate-spin"/> : <Save/>} CONFIRMAR</button>
            </div>
        </div>
      )}
      ...
      <div className="w-full max-w-[1400px] grid grid-cols-1 md:grid-cols-3 gap-4 items-center mb-8 relative z-10 mt-4">
        {/* TÍTULO (Izquierda en PC, Arriba en Móvil) */}
        ...
        {/* MONEDAS (CENTRO ABSOLUTO) - Aquí está la mejora */}
        ...
        <div className="flex gap-3 items-center justify-center md:justify-end">
            <SoundLink href="/profile" className="flex items-center gap-3 bg-slate-900/80 backdrop-blur-md pl-4 pr-1 py-1 rounded-full border border-slate-700 hover:border-pink-500 transition-all group shadow-lg">
               <span className="text-[10px] font-bold text-slate-300 group-hover:text-white hidden sm:block uppercase">{user ? user.displayName || 'JUGADOR' : 'PERFIL'}</span>
